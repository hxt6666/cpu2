...
MODULE_ID = "zygisk-next-module-test"
MODULE_NAME = "Zygisk Next Module Test"
VER_NAME = "1"
...
def build_zip(args):
    ...
    expand_text_file(module_path / 'zn_modules.txt', {
        'moduleId': MODULE_ID,
    })
    script_vars = {
...
def main():
    ...
    ap.add_argument('--ndk', dest='ndk', required=False, help="ndk version")
    ap.add_argument('--save-debug', dest='save_debug', action='store_true', help='save debug symbols as zip')
    ap.add_argument('--force', dest='force', help="Build without cache", action='store_true')
    ap.add_argument('-t', dest='build_type', help="Build type", choices=BUILD_TYPE_CHOICES, default=BUILD_TYPE_CHOICES[0])
    ...
    config_args = subps.add_parser('config', help='Generate compile_commands.json')
    config_args.set_defaults(func=config_cmd)
    config_args.add_argument('-a', dest='abi', choices=ABI_CHOICES, default=DEFAULT_ABI)

    deploy_args = subps.add_parser('deploy', help='Deploy executable to device')
    ...
    zip_args = subps.add_parser('zip', help='Build module zip')
    zip_args.set_defaults(func=zip_cmd)

    flash_args = subps.add_parser('flash', help='Build module and flash module to device')
